...
- [ ] 1.3 Write tests`;
  ...
  it('tools/list includes plan tools', async () => {
    ...
    expect(result.tools).toHaveLength(44); // 6 lock + 7 plan + 1 verify + 2 messaging + 4 roles + 6 phase1 + 8 phase2 + 5 phase3 + 1 phase4 (traces) + 1 eh_search_events (CIP phase 4) + 1 eh_purge_stale_agents + 1 eh_stop_all_workers
  });
