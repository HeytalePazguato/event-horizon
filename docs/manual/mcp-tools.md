...
| `eh_claim_handle` | Claim a short, workspace-unique name others can address you by |

### Two kinds of message

Every message carries a `kind`, which records **who sent it** — not how urgent or important it is:

- **`peer`** — sent by another Event Horizon-connected agent session through `eh_send_message`. The content can be anything: a finding, shared context, a bug report, a question, a heads-up that an API changed. Only sessions wired into Event Horizon (Claude Code, OpenCode, Copilot, Cursor via their connectors) can put a message in the queue; a tool outside EH has no way to send one.
- **`system`** — generated by Event Horizon itself (plan discovery, role instructions, worker-failure notices).

The split exists because the two have very different volumes. System notices are machine-generated and can outnumber agent traffic by hundreds to one, so without the distinction a peer message is unfindable.

`eh_get_messages` returns peer messages first, then system notices, capped at 50 by default. Filter with `kind`, `from_agent_id`, `exclude_from`, and `limit`:

```
eh_get_messages(agent_id: "...", kind: "peer")        # only messages from other agent sessions
eh_get_messages(agent_id: "...", exclude_from: "event-horizon")
```

Only the messages actually returned are marked read. Anything a filter or the limit held back stays unread and is reported in `pending`, so a narrow read can never silently consume mail you didn't see.
...
eh_send_message(agent_id: "...", to_agent_id: "csp", message: "...")
```

If that session restarts, the replacement claims `csp` again and collects everything queued for it in the meantime. A handle is refused only while another *running* agent holds it — once the holder stops, its replacement takes the name over. Agents that claim a plan task are given a handle automatically (`<role>-<task-id>`, e.g. `reviewer-2.1`), so orchestrated workers are addressable without doing anything.

Send to a handle when the conversation needs to outlive a session, and to an alias for anything within one. `to_agent_id` accepts any of the three, and `to_agent_name` targets by the runtime's own name:

```
eh_send_message(agent_id: "...", to_agent_name: "OpenCode", message: "...")
```

That works only while one agent of that runtime is running. When several are, the send is refused and the error lists their aliases so you can pick one — better than silently delivering to the wrong session.

### Getting told about new mail

An idle CLI session can't be woken from outside, so messages sit in the queue until the agent calls `eh_get_messages`. Event Horizon closes that gap through the `UserPromptSubmit` hook: whenever you send a prompt, the hook injects a one-line notice into that turn if peer mail is waiting.

```
[Event Horizon] You have 2 unread messages from IC agent. Call eh_get_messages(agent_id: "…", kind: "peer") to read them.
```

Nothing is injected when the inbox is empty, and peeking never marks anything read. Only peer messages trigger a notice — system notices are not worth interrupting a turn for. Hooks update themselves: Event Horizon rewrites every installed hook on each activation, so this arrives with the extension upgrade without you re-running setup.
