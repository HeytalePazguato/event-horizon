...
- **Dependency bumps**: `tree-sitter-php` 0.23.12 → 0.24.2, `tree-sitter-javascript` 0.23.1 → 0.25.0, `eslint` 10.2.1 → 10.3.0. `web-tree-sitter` stays on 0.25.x for this release; 0.26 dropped `./tree-sitter.wasm` from its `exports` map and requires a follow-up update to `scripts/copy-tree-sitter-wasm.mjs`. `@types/vscode` stays pinned to `^1.100.0` to match `engines.vscode` (vsce refuses to package when `@types/vscode > engines.vscode`).
